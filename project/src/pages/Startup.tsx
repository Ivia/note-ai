...
export default function Startup() {
  const { startupRecords, deleteStartupRecord } = useStore()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-base font-semibold text-gray-800">起号助手</h1>
        <button
      ...
      {startupRecords.length === 0 ? (
        <div className="text-center text-gray-400 py-20 text-sm">
          ...
          <p>还没有起号记录</p>
          <p className="mt-1">点击右上角「新起号」开始吧</p>
        </div>
      ) : (
        <div className="space-y-2">
          {startupRecords.map((record) => (
            <div
              ...
              onClick={() => navigate(`/startup/${record.id}`)}
            >
              <div>
                <p className="text-sm font-medium text-gray-800">
                  🏷️ {record.direction} · {record.goal}
                </p>
                {record.tags.length > 0 && (
                  <p className="text-xs text-gray-500 mt-0.5">{record.tags.join(' · ')}</p>
                )}
              ...
              <div className="flex items-center gap-2 ml-4 shrink-0">
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    if (confirm('确认删除该起号记录？')) deleteStartupRecord(record.id)
                  }}
